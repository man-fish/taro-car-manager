...
export default Behavior({
  ...
  data: {
    schema: "",
    tipFun: { message: "showMessage", toast: "showToast" },
    tipContent: { message: "content", toast: "title" },
    errorText: "",
    errors: []
  },
  methods: {
    ...
    getNeedValidateRule(t) {
      ...
      const r = t ? a.filter(e => e.trigger.indexOf(t) > -1) : a,
        s = new Schema({ [e]: r });
      return this.setData({ schema: s }), r;
    },
    validatorData(t, e) {
      const { tipType: a, tipFun: r, tipContent: s } = this.data;
        ...
        this.data.schema.validate(t, t => {
          if (
            ...
            const e = r[a],
              i = s[a];
            return "text" === a
              ? (this.setData({ errorText: t[0].message }), t)
              : wx.lin && wx.lin[e]
              ? (wx.lin[e] &&
                  wx.lin[e]({ [i]: t[0].message, duration: 1500, mask: !1 }),
                t)
              : (wx.showToast({
                  icon: "none",
                  title: `请在页面内引入${a}组件`
